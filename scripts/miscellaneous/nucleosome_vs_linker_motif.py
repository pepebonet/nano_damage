...
def annot(df):
    seq_pos = refseq('saccer3', df['CHROM'], df['Start'] - 1, 402)
    return seq_pos
...
def obtain_nuc_norm(df):
    ...
    all_pairs = []
    aa = list(ut.slicing_window(seq[0], 2))

    for i in range(len(aa)-1):
        all_pairs.append(aa[i:i+2])
    
    for i in tqdm(range(1, len(seq))):

        #Number of bases lower than 400 
        if len(list(seq[i])) < 400:
            continue
        
        aa = list(ut.slicing_window(seq[i], 2))

        for i in range(len(aa)-1):
            for el in aa[i:i+2]:
                all_pairs[i].append(el)

    norm = []
    #Count nucleotides for each base in a nucleosome
    for el in all_pairs:
        norm.append(Counter(el))
...
def main(damage, nucleosome_information, base_study, output):
    ...
    norms = obtain_nuc_norm(nucleosomes)
